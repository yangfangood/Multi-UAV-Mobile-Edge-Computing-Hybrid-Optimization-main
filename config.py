...
WPT_TRANSMIT_POWER: float = 500.0 * 1e6  # P^WPT in Watts (UAV WPT transmit power for energy harvesting) 无人机发射功率
...
ALPHA_2 = 0.4  # weightage for energy (negative term, lower priority than latency)  能耗权重（比延迟优先级低）
ALPHA_3 = 2.0  # weightage for fairness (positive term, encourage equal service)   公平性权重（鼓励公平服务）
ALPHA_4 = 50.0  # weightage for offline rate (negative term, penalizes UEs running out of battery) 离线率权重（强烈惩罚设备掉电）最怕设备没电掉线
...
INITIAL_NOISE_SCALE: float = 0.2 #初始探索噪声
